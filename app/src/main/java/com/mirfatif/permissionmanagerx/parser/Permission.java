...
public class Permission {
  ...
  public boolean isManifestPermAppOp() {
    return mManifestPermAppOp;
  }
  ...
  public CharSequence getPermNameString() {
    CharSequence permName = getPermLabel();
    if (mAppOp && hasDependsOnPerm()) {
      permName = TextUtils.concat(permName, " (", getDependsOnLabel(), ")");
    }
    return permName;
  }

  /**
   * The simple, readable label of this permission's name, e.g. {@code 读取联系人}. Unlike {@link
   * #getPermNameString()}, it never carries the {@code (parent)} suffix added to AppOps that depend
   * on a Manifest permission.
   */
  public String getPermLabel() {
    return PermNameMapper.INS.getLocalizedName(getName());
  }

  /** The readable label of the Manifest permission this AppOp depends on. */
  public String getDependsOnLabel() {
    CharSequence dependsOn = getDependsOnName();
    return dependsOn == null ? null : PermNameMapper.INS.getLocalizedName(dependsOn.toString());
  }
