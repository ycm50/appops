...
import com.mirfatif.permissionmanagerx.parser.PermNameMapper;
...
public class FilterSettingsFragment extends PreferenceFragmentCompat
  ...
  private Pair<String[], CharSequence[]> buildAppOpsNamesList() {
    ...
    for (int i = 0; i < labels.length; i++) {
      ...
      if (Constants.UNKNOWN_OP.equals(extraAppOp)) {
        ...
        labels[i] = PermNameMapper.INS.getLocalizedName(extraAppOp);
      }
    }
  ...
  private void updateExcludedPermsView(
    ...
    // The stored values stay the raw permission names; only what is shown is translated.
    CharSequence[] permEntries = new CharSequence[excludedPerms.length];
    for (int i = 0; i < excludedPerms.length; i++) {
      permEntries[i] = PermNameMapper.INS.getLocalizedName(excludedPerms[i].toString());
    }

    excludedPermsListView.setEntries(permEntries);
    ...
    if (permCount == 0) {
      ...
      String message = PermNameMapper.INS.getLocalizedName(excludedPerms[0].toString());
  ...
  private void updateExtraAppOpsView(
    ...
    if (extraAppOpsCount == 0 || appOpsCount == 0) {
      ...
      message = PermNameMapper.INS.getLocalizedName((String) extraAppOps.toArray()[0]);
      extraAppOpsCount--;
