...
public enum MySettings {
  ...
  /**
   * True once a native ADB connection has been established, which is the only case in which {@link
   * #getAdbHost()} and {@link #getAdbPort()} hold something the user actually paired instead of the
   * built-in defaults. {@link com.mirfatif.permissionmanagerx.svc.AdbConnectSvc} saves the port
   * only after a successful connect.
   */
  public boolean isAdbPortSaved() {
    return getDefPrefs().contains(getString(R.string.pref_privs_adb_port_key));
  }
