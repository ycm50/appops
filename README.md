...
* 点某个权限进入下一级，列出正在使用它的应用；每一行都能直接开关，行内开关只对可修改的项生效。
* 单权限页面菜单里有**全部开启 / 全部关闭**：对当前列出的应用中「状态确实需要改」的那些批量生效，执行前弹窗确认，并复述上游的位置（系统应用 / 框架应用）警告；不可修改的项与 UID 模式 AppOp 会被跳过（后者在列表里本来就是只读的文本状态），改完弹出实际修改的数量。
* 数据由 `PermViewParser` 从 `PackageParser` 的包列表聚合而来，因此用户在**排除过滤器**里配置的应用/权限规则在这里同样生效，两个界面看到的内容始终一致。
* **设置 → 常规设置 → 显示系统应用**（默认开）是全局开关：关掉后系统应用（含框架应用）从主界面（按应用管理）的应用列表里消失，权限视图也自然不再统计它们，下一级应用列表同样看不到。实现上只改了 `PackageParser` 的包过滤，两个界面都从同一份包列表派生，因此不会出现「一个界面有、另一个没有」。
* 它和过滤器里的「排除系统应用」是两个开关、都能藏系统应用：过滤器那个属于过滤器组（受过滤器总开关控制），设置里这个不依赖过滤器总开关。系统应用只有在「过滤器没排除」且「设置里开着显示」时才出现——显式排除不会被另一个开关悄悄覆盖。
* 隐藏系统应用时**刷新也不会去扫描它们**：包列表在读取应用标签（每个应用一次 IPC）和解析权限之前就把系统应用整个丢掉，进度条只统计真正会列出的应用，系统应用那一堆签名比对与特权解析也一并省掉。缓存会记住构建它时的过滤状态，所以改开关（或改过滤器的「排除系统应用」）时会自动失效并重新扫描，不会出现「打开开关却看不到系统应用」的残留。
